...
class FanDiagram(QtGui.QWidget):
    ...
    Or, if alphaData and gammaData are strings, assumes they're paths to data
        files to plot
    :param alphas:
    :param gammas:
    :param kwargs:
    ...
    def export(self, fname, hideHistograms=True, pngScale=4):
        ...
        If fname.endswith(".svg"), it outputs as an SVG. Howver, it's not the
            ...
            converted to pdf via Inkscape, things get fucked up for some
    ...
    def hideHistogramAxes(self, hideTicks=True):
        ...
        # Keep a reference to the old paint methods so you can reverse it if
        # desired This stops the painting of the bars which go from the linear
        # region to the gradient editor
        self.histGamma.item.oldPaint = self.histGamma.item.paint
        self.histAlpha.item.oldPaint = self.histAlpha.item.paint
        # Overwriting the functions to return None causes all the other
        # rendering things to abort
        ...
        if hideTicks:
            ...
            # Note: This only hides ticks which are present
            [ii.hide() for ii in self.histAlpha.item.gradient.ticks.keys()]
            [ii.hide() for ii in self.histGamma.item.gradient.ticks.keys()]

        QtWidgets.QApplication.processEvents()
        # Hard coded numbers which make it look like the axes values line up
        # with the gradient item, which is more in-line with how color bars are
        # interpreted
        self.histGamma.axis.setRange(-46.75, 46.75)
        self.histAlpha.axis.setRange(-94, 94)
    ...
    def fromTMatrix(tMatrix, angle=45, sbs=None):
        """
        Create a fan diagram from T matrices directly. The angle needs to be
            specified so the T matrix can be converted to a J matrix. The angle
            is relative to what's specified in the qwp.extractMatrices.makeU
            function.

        if you pass a string, it assumes it's a file name from a saved one.
        It'll load that and plot it. If you also pass values to sbs, it'll make
        sure only the passed values are plotted. Otherwise, it'll plot all the
        sbs in the file

        If you pass a tMatrix as returned from the fitting routines, you also
        need to pass the sbs directly in this case, since the tMatrices don't
        include them.
        ...
        if isinstance(tMatrix, str):
            ...
            if wantsbs is not None:
                try:
                    # Find the indices of the desired sidebands within the
                    # array of sidebands actually loaded
                    wantIdx = [sbs.tolist().index(ii) for ii in wantsbs]
                    # Cull it to only those specified
                    sbs = sbs[wantIdx]
                    # tMatrix is multidimensional (tMatrix.ndim>2), so ellipses
                    # cut out the other axes
                    tMatrix = tMatrix[..., wantIdx]
                    # Ensure that you got everything you want. Could happen if
                    # sidebands are requested (passed to the function) and not
                    # found
                    assert np.all(wantsbs == sbs)
                except ValueError as e:
                    raise IndexError(
                        "Invalid sideband requested ({} is not in loaded)"
        ...
        if sbs is None:
            raise RuntimeWarning("Desired sidebands to plot should be"
                                 "specified as kwarg sbs")
    ...
    def setTitle(self, title="", adjustBounds=True):
        """
        Sets the title of the fan diagram, positioning the text right above the
            center of the fan
        :param title:
        :param adjustBounds:
        :return:
        ...
        QtWidgets.QApplication.processEvents()
        # Double up because of some weird fucking issue with Qt not
        # appropriately updating things when requested
        self.titleItem.setPos(0, self.axes["azimuthal"].fullBoundingRect.top())
        QtWidgets.QApplication.processEvents()
        ...
        if adjustBounds:
            ...
            top = self.titleItem.mapRectToView(
                self.titleItem.boundingRect()).top()
    ...
    def setMaxRadius(self, radius=40):
        ...
        # Need to invalidate the cache for the axes, forcing it to redraw and
        # update the bounding rect and stuff

...
class NeonNoiseAnalysis(CCD.CCD):
    ...
    def __init__(self, fname, spectrometer_offset=None):
        # print 'opening', fname
        super(NeonNoiseAnalysis, self).__init__(
            fname, spectrometer_offset=spectrometer_offset)
    ...
    def noise_and_signal(self):
        ...
        # for dark current measurements
        self.high_noise_region = np.array(self.ccd_data[80:180, :])
        ...
        # self.noise_list = [
        #   self.high_noise, self.low_noise1, self.low_noise2, self.low_noise3]
        ...
        self.error1 = np.sqrt(np.sum(
            self.peak1[temp_max - 1:temp_max + 2, 2] ** 2))

        temp_max = np.argmax(self.peak2[:, 1])
        self.signal2 = np.sum(self.peak2[temp_max - 1:temp_max + 2, 1])
        self.error2 = np.sqrt(np.sum(
            self.peak2[temp_max - 1:temp_max + 2, 2] ** 2))

        temp_max = np.argmax(self.peak3[:, 1])
        self.signal3 = np.sum(self.peak3[temp_max - 1:temp_max + 2, 1])
        self.error3 = np.sqrt(np.sum(
            self.peak3[temp_max - 1:temp_max + 2, 2] ** 2))

        temp_max = np.argmax(self.peak4[:, 1])
        self.signal4 = np.sum(self.peak4[temp_max - 1:temp_max + 2, 1])
        self.error4 = np.sqrt(np.sum(
            self.peak4[temp_max - 1:temp_max + 2, 2] ** 2))

        temp_max = np.argmax(self.peak5[:, 1])
        self.signal5 = np.sum(self.peak5[temp_max - 1:temp_max + 2, 1])
        self.error5 = np.sqrt(np.sum(
            self.peak5[temp_max - 1:temp_max + 2, 2] ** 2))

        self.signal_list = [
            self.signal1, self.signal2, self.signal3,
            self.signal4, self.signal5]
        self.error_list = [
            self.error1, self.error2, self.error3,
            self.error4, self.error5]
        print("Signal list:", self.signal_list)
        self.ccd_data = np.flipud(self.ccd_data)

    def process_stuff(self):
        """
        This one puts high_noise, low_noise1, signal2,
        and error2 in a nice horizontal array
        """
        # self.results = np.array(
        #    [self.high_noise, self.low_noise1, self.signal5, self.error5])
        # average = np.mean(
        #    [self.low_noise1, self.low_noise2, self.low_noise3])
        # self.results = np.array(
        #    [self.high_noise, self.low_noise1, self.low_noise2,
        #    self.low_noise3, self.high_noise/average])
        self.results = np.array(
            [self.high_noise_sig, self.high_noise_std, self.low_noise_sig,
             self.low_noise_std])


def collect_noise(
 neon_list, param_name, folder_name, file_name, name='Signal'):
    """
    ...
    param_array = np.column_stack((
        param_array, param_array[:, 4] / param_array[:, 3]))
    # append the snr
    param_array = np.column_stack((
        param_array, param_array[:, 3] / param_array[:, 2]))
    ...
    origin_import1 = (
        param_name +
        ",Noise,Noise,Signal,error,rel peak error,peak signal-to-noise")
    ...
    # TODO: reduce length of below line
    origin_import3 = ",High noise region,Low noise region,{},{} error,{} rel error, {}".format(name, name, name, name)
    # origin_import3 = (
    #   ",High noise region,Low noise region 1," +
    #   "Low noise region 2,Low noise region 3,High/low")
    header_total = (
        origin_import1 + "\n"
        + origin_import2 + "\n"
        + origin_import3)
    ...
    np.savetxt(
        os.path.join(folder_name, file_name), param_array, delimiter=',',

...
def get_alphagamma(file,observedSidebands,crystalAngle,saveFileName,save_results=False):
    """
    ...
    fanData = qwp.expFanCompiler.FanCompiler(observedSidebands,keepErrors = True)
...
# These are some things to set before hand. I picked this from some data we took
#   a few weeks ago.
observedSidebands = np.arange(8,32,2)
crystalAngle = 16
saveFileName = "12-05_alphasgammas" # Feel free to change this one, don't change the other two

alphaData,gammaData = get_alphagamma(r"Fan Data",observedSidebands,16,
    saveFileName,save_results = False)

# Now we need to calculate the Jones matrices.
# For this function you need to cut out errors, and just feed it an array of sb, alpha
# I leave that for you to figure out
J = qwp.extractMatrices.findJ(alphas,gammas)

# and save the matrices to text
qwp.extractMatrices.saveT(J, observedSidebands, "{}_JMatrix.txt".format(saveFileName))
